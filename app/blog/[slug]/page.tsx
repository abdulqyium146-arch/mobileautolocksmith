  ...
  'transponder-keys-explained': {
    ...
    content: (
      <div className="prose-content">
        ...
        <p>No — this is one of the key distinctions between a specialist auto locksmith and a general locksmith. The programming requires OBD-level diagnostic equipment with the appropriate manufacturer software licences. Our investment in professional-grade automotive programming tools is what enables Ian and Adam to programme all major makes and models on-site across the North West.</p>
        ...
        <h2>Summary</h2>
        <p>A transponder key is a car key containing an electronic chip that communicates with the vehicle&apos;s immobiliser. Every modern vehicle requires the transponder to be programmed to the specific car&apos;s ECU before the engine will start. This programming requires specialist equipment and cannot be done by a general key-cutting shop. For North West drivers, Mobile Auto Locksmith handles all transponder key types for all major makes — call <a href="tel:07831872740">{SITE.phone}</a> for assistance.</p>
      </div>
    ),
  },
}
